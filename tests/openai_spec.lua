...
describe("ChatGPT call", function()
    ...
    it("uses the correct API key and body", function()
        ...
        local on_delta = function(_) end

        env.OPENAI_API_KEY = "test"

        curl.post.returns({ body = vim.fn.json_encode(completion_response) })

        openai.get_chatgpt_completion(config.options, { { role = "system", content = "Say hello!" } }, on_delta)

        assert.stub(curl.post).was_called_with(
